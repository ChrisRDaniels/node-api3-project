...
router.post("/:id/posts", validateUserId, validatePost, (req, res) => {
 const post = req.body;
 postdb
  .insert({ user_id: req.params.id, text: post.text })
  .then(post => {
 ...
 userdb
  ...
  .catch(err => {
   ...
   res
    .status(500)
    .json({
     message: "There was an error retrieving a user from the database."
    });
  });
});
...
router.delete("/:id", validateUserId, (req, res) => {
 userdb
  ...
  .catch(err => {
   ...
   res
    .status(500)
    .json({ message: "There was an error deleting a user from the database." });
  });
});
...
router.put("/:id", validateUserId, (req, res) => {
 userdb
  ...
  .catch(err => {
   ...
   res
    .status(500)
    .json({ message: "There was an error updating the user in the database." });
  });
});
